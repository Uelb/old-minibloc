...
$(document).ready(function(){
  ...
  if($('.notice').size() != 0){
    $('.notice').hide();
    setTimeout(function() {noty({text: '<strong>' + $('.notice').html() + '</strong>',layout:'topRight',type:'success', timeout: 5000});});
  }
  if($('.alert').size() != 0){
    $('.alert').hide();
    setTimeout(function() {noty({text: '<strong>' + $('.alert').html() + '</strong>',layout:'topRight',type:'error', timeout: 5000});});
  }
});
